...
void Eval_DT(ZZ &Tb, ZZ_p &yb, int B, ep_t &g1T1, ep2_t &g2T2, int model, int d, PVHSS_Para pvhssPara, PKE_Para pkePara,
...
void Eval_DT(int model, int d, int cyctimes)
{
    PKE_Para pkePara;
    ...
    {
        time = GetTime();
        PVHSS_Ver(y, y1, y2, g1T1, g2T2, pvhssPara);
        Time[i] = GetTime() - time;
    }
    cout << "Verification passed!\nThe value of function: " << y << "\n";
    DataProcess(mean,stdev,Time,cyctimes);
