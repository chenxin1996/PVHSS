...
int main() {
    ...
    cout << "************************Benchmarks (Section 5.1 in the paper)************************" <<"\n";
    // the size of the input (i.e. the value of Log(B_msg) in the paper),
    // and it set to 2,16,32,64,128,256,512,1024 in the experiments in our paper.
    int msg_bit=64;
    ...
    // test the running time of decryption algorithm
    Time_Dec(msg_bit,  cyctimes);
    // test the running time of each subroutine in evaluation algorithm
    Time_Eval_Subalgo(msg_bit,  cyctimes);

    cout << "\n\n**************Multivariate polynomial evaluation (Section 5.2 in the paper)**************" <<"\n";
